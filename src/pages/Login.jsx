...
const Login = () => {
    ...
    const [aLoading, setLoading] = React.useState(false);
    ...
    // If user is logged in, navigate to dashbord
    React.useEffect(() => {
        if (user) {
            naveget('/dashbord')
        }
    ...
    React.useEffect(() => {
        if (loading) return;
        if (user) window.location.href = '/dashbord';
    }, [user, loading]);
                    ...
                    <div className="p-6 sm:p-8 rounded-2xl bg-white border border-gray-200 shadow-sm">
                        ...
                        <form onSubmit={handleRegister} className="mt-12 space-y-6">
                            ...
                            <div>
                                ...
                                <div className="relative flex items-center">
                                    <input onChange={(e) => setFormData({ ...formData, password: e.target.value })} name="password" type="password" required className="w-full text-slate-900 text-sm border border-slate-300 px-4 py-3 pr-8 rounded-md outline-blue-600" placeholder="Enter password" />
                            ...
                            <div className="!mt-12">
                                {
                                    aLoading ? <button desabled className="w-full py-2 px-4 text-[15px] font-medium tracking-wide rounded-md text-white bg-blue-600 hover:bg-blue-700 focus:outline-none cursor-not-allowed">
                                        جاري الدخول ...
                                    </button> : <button type="submit" className="w-full py-2 px-4 text-[15px] font-medium tracking-wide rounded-md text-white bg-blue-600 hover:bg-blue-700 focus:outline-none cursor-pointer">
                                        تسجيل الدخول
                                    </button>
                                }
                            </div>
