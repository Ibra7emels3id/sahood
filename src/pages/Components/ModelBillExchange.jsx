...
const ModelBillExchange = ({ showModelBillExchange, setShowModelBillExchange, FetchDataSafe }) => {
  ...
  `);
    ...
    const HandleSubmitData = async (e) => {
        ...
        try {
            setLoading(true);
            const res = await axios.put(`${import.meta.env.VITE_SOME_URL}/api/bill-exchange/${user.uid}`, FormData);
            toast.success(res.data.message);
            handlePrint(FormData);
            setShowModelBillExchange(false);
            FetchDataSafe();
            setFormData({
                name: "",
                amount: "",
    ...
    useEffect(() => {
        if (showModelBillExchange) {
            document.body.style.overflow = 'hidden';
        } else {
            document.body.style.overflow = 'auto';
        }
    }, [showModelBillExchange]);


    return (
        <div className={showModelBillExchange ? 'block' : 'hidden'}>
            <div class="fixed inset-0 p-4 flex flex-wrap justify-center items-center w-full h-full z-[1000] before:fixed before:inset-0 before:w-full before:h-full before:bg-[rgba(0,0,0,0.5)] overflow-auto">
                <div class="w-full max-w-lg bg-white shadow-lg rounded-md p-8 relative">
                    <svg onClick={() => setShowModelBillExchange(false)} id="closeIcon" xmlns="http://www.w3.org/2000/svg"
...
export default ModelBillExchange;
